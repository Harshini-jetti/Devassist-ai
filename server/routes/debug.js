...
import { debugErrorPrompt } from '../prompts/promptTemplates.js';
...
router.post('/', async (req, res) => {
  ...
  if (!code || code.trim() === '' || !errorMessage || errorMessage.trim() === '') {
    return res.status(400).json({ error: 'Please paste both your code and the error message.' });
  }

  try {
    const prompt = debugErrorPrompt(code, errorMessage);
